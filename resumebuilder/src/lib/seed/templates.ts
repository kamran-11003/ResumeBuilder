...
export async function seedTemplates() {
  try {
    ...
    // Check if Deedy template already exists
    const existingTemplate = await Template.findOne({ name: 'Deedy Resume' });
    
    if (existingTemplate) {
      console.log('Deedy template already exists, skipping...');
      return;
    }

    // Create Deedy template
    const deedyTemplateDoc = new Template({
      name: 'Deedy Resume',
      description: 'A professional two-column resume template with clean typography and modern layout. Perfect for technical professionals and students.',
      category: 'professional',
      author: 'Debarghya Das & Zachary Taylor',
      version: '1.3',
      latexCode: deedyTemplate,
      isActive: true,
      isPremium: false,
      tags: ['professional', 'two-column', 'technical', 'clean', 'modern'],
      metadata: {
        sections: [
          'personal_info',
          'experience',
        ...
        features: ['two-column', 'clean-typography', 'professional-layout']
      },
      usage: {
        totalUses: 0
      }
    ...
    await deedyTemplateDoc.save();
    console.log('✅ Deedy template seeded successfully!');

    // Add more default templates here if needed
